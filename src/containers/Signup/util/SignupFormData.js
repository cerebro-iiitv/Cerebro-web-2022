...
export const validatePage = (values, page) => {
  ...
  if (page === 1) {
    if (!values.firstname) errors.firstname = "First name is required";
      ...
      errors.firstname = "First name should be less than 30 characters";
    if (!values.lastname) errors.lastname = "Last name is required";
    else if (values.lastname.length > 30)
      errors.lastname = "Last name should be less than 30 characters";
    if (!values.phone) errors.phone = "Phone number is required";
    else if (!values.phone.match(/^\d{10}$/))
      errors.phone = "Please enter a valid 10 digit mobile number";
    if (!values.address) errors.address = "Address is required";
    else if (values.address.length > 225)
      errors.address = "Address should be less than 225 characters";
    ...
    else if (values.institute.length > 225)
      errors.institute = "Institute name should be less than 225 characters";
    if (!values.degree) errors.degree = "Degree is required";
    else if (values.degree.length > 30)
      errors.degree = "Degree should be less than 30 characters";
